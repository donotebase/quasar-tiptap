    ...
    >
      <q-list>
        <template v-for="(item, index) in essentialLinks">
          <div :key="index">
            <q-item-label header class="text-grey-8" v-if="item.groupLabel">
              {{item.groupLabel}}
            </q-item-label>
            <essential-link v-bind="item" />
          </div>
        </template>
      </q-list>
    </q-drawer>
  ...
  data () {
    return {
      ...
      essentialLinks: [
        ...
        { title: 'Basic', caption: 'quasar.dev', icon: 'school', type: 'route', to: '/tiptap/test1', groupLabel: 'quasar-tiptap' },
